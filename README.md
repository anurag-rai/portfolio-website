...
The site is optimized for search engines with the following:

### Meta Tags (Layout.astro)

- `<meta name="description">` -- site description from profile data
- `<meta name="robots" content="index, follow">` -- explicitly allows crawling and indexing
- `<meta name="theme-color" content="#0f0f0f">` -- browser theme color matching the site background
- `<link rel="canonical">` -- canonical URL via `Astro.url` to prevent duplicate content

### Open Graph

- `og:title`, `og:description`, `og:type` (set to `profile`), `og:locale`, `og:url`, `og:site_name`
- No `og:image` currently -- add a 1200x630 PNG to `public/` and reference it in Layout.astro when ready

### Twitter Card

- `twitter:card` (set to `summary`), `twitter:title`, `twitter:description`
- Upgrade to `summary_large_image` when an og:image is added
...
- Allows all crawlers, references the sitemap URL

### Structured Data (JSON-LD)

- `Person` schema in Layout.astro head
- Includes: name, job title, site URL, email, GitHub, and LinkedIn
- Validates at [Google Rich Results Test](https://search.google.com/test/rich-results)

### What's Not Included (by design)

- **og:image / twitter:image** -- no social preview image yet; add one to `public/` and update Layout.astro
- **Personal address, phone, photo** -- intentionally excluded to avoid PII exposure
- **Google Analytics / tracking** -- not added; consider adding if analytics are needed
